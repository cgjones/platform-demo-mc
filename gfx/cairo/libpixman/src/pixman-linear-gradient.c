...
uint16_t convert_8888_to_0565(uint32_t color)
{
...
uint16_t dither_8888_to_0565(uint32_t color, pixman_bool_t toggle)
{
			...
			const uint32_t *mask)
{
	...
	if (((pixman_fixed_32_32_t )(inc * width)) == 0)
	{
	    ...
	    color = _pixman_gradient_walker_pixel (&walker, t);
	    while (buffer < end) {
		// we should only dither twice instead of for every pixel
		*buffer++ = dither_8888_to_0565(color, toggle);
		toggle ^= 1;
	    }
	}
	else
