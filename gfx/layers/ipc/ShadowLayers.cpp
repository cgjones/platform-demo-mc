...
ShadowLayerForwarder::EndTransaction(InfallibleTArray<EditReply>* aReplies)
{
  ...
  for (ShadowableLayerSet::const_iterator it = mTxn->mMutants.begin();
    ...
    common.scalingMatrix() = mutant->GetScalingMatrix();
...
ShadowLayerForwarder::ShadowDrawToTarget(gfxContext* aTarget) {
  ...
  nsRefPtr<gfxASurface> surface = OpenDescriptor(OpenReadWrite, descriptorOut);
...
{
  if (!mSurfaceAsImage) {
    mSurfaceAsImage = mSurface->GetAsImageSurface();
  }
