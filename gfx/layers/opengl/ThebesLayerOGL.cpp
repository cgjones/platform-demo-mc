...
#ifdef ANDROID
  void BindGralloc(const SurfaceDescriptor& aSurface, const nsIntRect& aRect, const nsIntPoint& aRotation);
  void UnbindGralloc();
#endif
...
void
ShadowBufferOGL::BindGralloc(const SurfaceDescriptor& aSurface, const nsIntRect& aRect, const nsIntPoint& aRotation)
{
  AutoOpenSurface surface(OpenReadOnly, aSurface);
  EnsureTexture(surface.Size(), surface.ContentType());
  mInitialised = true;
  sp<GraphicBuffer> buffer =
    GrallocBufferActor::GetFrom(aSurface.get_SurfaceDescriptorGralloc());
  mTexImage->BindGralloc(buffer->getNativeBuffer());
  mBufferRect = aRect;
  mBufferRotation = aRotation;
}

void
ShadowBufferOGL::UnbindGralloc()
{
  mTexImage->UnbindGralloc();
}
  ...
  , mUploadTask(nsnull)
{
  ...
  NS_RUNTIMEABORT("Can't use non-tiled thebes layer and thebes layer.");
#endif
  mImplData = static_cast<LayerOGL*>(this);
}
                           ...
                           nsIntRegion* aFrontUpdatedRegion)
{
  ...
  if (SurfaceDescriptor::TSurfaceDescriptorGralloc == aNewFront.buffer().type()) {
    AutoOpenSurface newFrontBuffer(OpenReadOnly, aNewFront.buffer());

    if (IsSurfaceDescriptorValid(mFrontBuffer.buffer())) {
      AutoOpenSurface currentFront(OpenReadOnly, mFrontBuffer.buffer());
    ...
    if (mBuffer) {
      mBuffer->UnbindGralloc();
        
      *aNewBack = mFrontBuffer;
      *aNewBackValidRegion = mOldValidRegion;

      // XXX why are we doing this?
      nsRefPtr<ShadowBufferOGL> temp = mBackBuffer;
      mBackBuffer = mBuffer;
      mBuffer = temp;
    } else {
      *aNewBack = null_t();
    ...
    *aFrontUpdatedRegion = aUpdatedRegion;
    ...
    mOldValidRegion = aUpdatedRegion;

    if (!mBuffer) {
      mBuffer = new ShadowBufferOGL(this);
    }
    mBuffer->BindGralloc(mFrontBuffer.buffer(), mFrontBuffer.rect(), mFrontBuffer.rotation());
  } else {
    if (!mBuffer) {
      mBuffer = new ShadowBufferOGL(this);
    }
    AutoOpenSurface frontSurface(OpenReadOnly, aNewFront.buffer());
