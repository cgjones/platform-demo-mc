...
ShadowThebesLayerOGL::EnsureTextureUpdated()
{
  ...
  AutoOpenSurface frontSurface(OpenReadOnly, mFrontBuffer.Buffer());
...
ShadowThebesLayerOGL::EnsureTextureUpdated(nsIntRegion& aRegion)
{
  ...
  for (int i = 0; i < 4; i++) {
    ...
    AutoOpenSurface surface(OpenReadOnly, mFrontBuffer.Buffer());
                           ...
                           nsIntRegion* aFrontUpdatedRegion)
{
  ...
  if (ShouldDoubleBuffer()) {
    ...
    if (IsSurfaceDescriptorValid(mFrontBufferDescriptor)) {
      AutoOpenSurface currentFront(OpenReadOnly, mFrontBufferDescriptor);
    ...
    if (!mDestroyed) {
      ...
      AutoOpenSurface frontSurface(OpenReadOnly, mFrontBufferDescriptor);
